// src/components/DenyAccessModal.tsx
// Modal for formally denying access and escalating to SOC Tier 3

import React, { useState } from 'react';
import { XCircle, X, Info, AlertTriangle } from 'lucide-react';
import { AccessRequestItem } from '../types';
import { SIMULATED_ENFORCEMENT_LABEL } from '../constants';

interface DenyAccessModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export function DenyAccessModal({
  isOpen,
  onClose,
  request,
  reviewerName,
  reviewerRole,
  onConfirm,
}: DenyAccessModalProps) {
  const [reason, setReason] = useState<string>(
    'Emergency access cannot be justified. Requested scope exceeds runbook limits or alternative low-privilege routes are available.'
  );
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!reason.trim()) {
      setError('Please provide a formal justification for denial.');
      return;
    }
    onConfirm({
      reviewerName,
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-150">
      <div className="relative w-full max-w-lg bg-[#0b0f19] border border-white/15 rounded-2xl shadow-2xl overflow-hidden">
        <div className="p-5 border-b border-white/10 bg-gradient-to-r from-rose-950/40 via-[#0b0f19] to-[#0b0f19] flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 mt-0.5">
                Zero permissions will be granted and request will terminate
              </p>
            </div>
          </div>
          <button
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4 text-xs">
          <div>
            ...
            <textarea
              ...
              rows={4}
              placeholder="State the compliance, operational or security rationale for denial..."
